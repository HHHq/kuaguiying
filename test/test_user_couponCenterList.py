...
excel_data = ExcelHandler(config.excel_path).read_sheet("用户端_领券中心列表")
request_url = read_yaml(config.yaml_path)["env_user"]["url"]
logger = MidWare.logger


@ddt.ddt
class TestUserCouponCenterList(unittest.TestCase):
    '''用户端_领券中心-列表模块'''
    ...
    @ddt.data(*excel_data)
    # @unittest.skip("111")
    def test_user_couponCenterList(self, info):
        ...
        try:
            ...
            logger.info("用户端_领券中心列表_测试用例id_{}通过".format(info["case_id"]))
        except Exception as e:
            logger.error("用户端_领券中心列表_测试用例id_{}不通过".format(info["case_id"]))
            raise e
